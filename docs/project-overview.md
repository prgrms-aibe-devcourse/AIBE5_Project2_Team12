...
2026-04-27 기준으로는 현재 리포와 현재 `ERDCloud` 초안을 우선 기준으로 삼고, 이전에 논의했던 확장 설계는 별도 보류 항목으로 취급한다.
...
현재 구현 기준으로는 아래 순서로 읽는 편이 가장 빠르다.
...
- 파일 기반 계약/완료 증빙 업로드와 `proposal_attachments`는 현재 보류다.

## 2026-04-27 구현 스냅샷

- 계정, 이력서, 스킬, 프로필 이미지, 이력서 첨부, 제안서, 모집 포지션, 포지션 요구 스킬, 매칭, 추천 실행/결과, 이력서 임베딩, AI 인터뷰 메시지 엔티티가 코드에 반영돼 있다.
- 클라이언트/프리랜서 대시보드, 제안서 작성/상세/수정, 추천 실행/상태/결과, 매칭 상세/목록 흐름이 템플릿과 컨트롤러까지 연결돼 있다.
- AI 브리프와 AI 인터뷰는 `WRITING` 상태 제안서를 점진적으로 채우는 흐름으로 구현돼 있으며, AI 인터뷰 대화 이력은 별도 테이블에 저장된다.
- 추천 MVP는 `recommendation_runs`, `recommendation_results`, `resume_embeddings`와 스케줄러 기반 비동기 처리로 동작한다.
- `local`/`demo` 프로필에서는 50명 프리랜서, 시드 제안서 2건, 샘플 매칭, 사전 계산된 추천 결과가 자동 적재된다.
...
현재 구현에서는 AI 인터뷰 대화도 같은 초안 흐름에 연결된다.
`proposal_ai_interview_message`에 사용자/AI 메시지를 순차 저장하고, 누적 대화 내용을 `proposal.raw_input_text`와 제안서 초안에 다시 동기화한다.
...
- 스킬 도메인
- 파일 업로드 및 로컬 저장
- `ProfileImage`, `ResumeAttachment`, `StoredFile` 기반 프로필/이력서 자산 연결
- 클라이언트 대시보드와 필터링 카드
- 프리랜서 대시보드와 매칭 요약 카드
...
- AI 인터뷰 메시지 저장, 대화 기반 초안 갱신, 드래프트 동기화
- 추천 실행 생성/재사용, 실행 상태 화면, 실행 이력, 결과 상세, 추가 추천
- `RecommendationRun`, `RecommendationResult`, `ResumeEmbedding` 기반 추천 저장 모델
- 매칭 요청, 수락/거절, 계약 시작 확인, 취소 요청/철회/확인, 후기 작성, 완료 확인
- 매칭 상세 화면과 제안서별 매칭 목록/필터 화면
- `local`/`demo` 프로필 시드 데이터와 추천/매칭 샘플 시나리오

### 남은 주요 작업

- 추천 품질 고도화와 운영 지표 가시성 보강
- 추천 외 경로의 프리랜서 직접 지원 진입점
- 운영자용 직무/스킬/추천 품질 관리 화면
- `proposal_attachments`, 계약서/완료 증빙 같은 파일 기반 확장 도메인
